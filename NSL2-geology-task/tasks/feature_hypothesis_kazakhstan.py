...
class FeatureHypothesisKazakhstanTask(TaskSpec[FeatureHypothesisKazakhstanState]):
    ...
    def populate(
        ...
        # Once all sources are visited, attempt greedy BIC init (no-op if
        # already complete or another episode beat us to it).
        if all_sources_done:
            ...
            steps=(
                ...
                WorkflowStep(
                    ...
                    prompt=(
                        ...
                        "   **For features with areal or volumetric extent (stratigraphic horizons, redox fronts, formation volumes):**\n"
                        ...
                        "   ⚠️  NEVER fill the entire grid (all of lon AND lat AND depth at once) — a constant layer has zero spatial signal and will score 0.\n"
                        "   ALWAYS constrain at least one dimension: a partial lon/lat extent derived from your analysis, OR a specific depth slice, OR both.\n\n"
                        ...
                        "      • search_web_geological('<formation name> geological formation Kazakhstan')\n"
                        "      • search_geonames_lookup('<map sheet>', 'Kazakhstan')\n"
                        "   3. If search yields coordinates → use them\n"
                        "   4. If search fails or is ambiguous → reason from the analysis findings:\n"
                        "      • directional terms ('southeastern', 'northern edge', 'central') → place in the corresponding portion of the grid\n"
                        "      • When in doubt, distribute spatially across the area your hypothesis describes\n"
                        "3. Create exactly ONE coherent feature layer:\n"
                        "   - ALL spatial operations must use the SAME layer name\n"
                        "   - Values must be floats or booleans representing the geological signal strength\n"
...
'''
    ...
    def _exec_scoring_capability(
        ...
        try:
            ...
            if result.get("success"):
                ...
                # Update translate phase record — prefer the timestamped name
                # returned by the scoring tool (result["layer_name"]) over the
                # bare name the agent passed in (args["name"]), which lacks the
                # timestamp suffix appended by evaluate_new_layer.
    ...
    def _run_greedy_bic_initialization(
        ...
        Guarded by greedy_init_complete.json + _kg_lock so only one of the
        parallel episodes ever executes this. O(N^2) = 153 evals for N=18.

        Round 1: pick the layer with the highest null-model BIC (most spatially
        variable → richest information content as a foundation).
        Round 2+: greedily add whichever remaining layer most reduces the
        geological_coherence_score (most complementary to the current selection).

        Writes greedy_init_complete.json with the list of selected layer names.
        Non-selected layers stay in the store but are excluded from crossbreeding
        by the flag check in populate().
        """
        ...
        with self._kg_lock(kg_dir):
            ...
            import sys as _sys
            vfm_path = str(Path(__file__).parent.parent.parent / "voxel-features-mcp")
            if vfm_path not in _sys.path:
                _sys.path.insert(0, vfm_path)
